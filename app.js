...
function drawKitten(kitten) {
  if (kitten.affection <= 0) {
    return `
    <div class=" edges kitten ${kitten.mood} card text-light bg-dark p-3 m-1">
    <div class=" edges text-center mb-1">
...
function checkStarted() {
  if (kittens.length) {
    document.getElementById("welcome").firstElementChild.innerHTML += `<button class="btn-cancel edges" onclick="clearKittens()">Clear ${kittens.length} Kittens</button>`
  }
}
function headerBtn(){
  if (kittens.length) {
  document.getElementById("header").firstElementChild.innerHTML += `<button class="btn-cancel edges" onclick="clearKittens()">Clear ${kittens.length} Kittens</button>`
  }
  }
